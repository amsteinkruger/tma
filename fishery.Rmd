...
fun_a_n = function(a){121.99 * 10 ^ (-0.09 * a)}
...
plot_weib =
ggplot() + 
  geom_line(aes(seq(1, 26), v_select_fit), color = "Firebrick4") + 
...
bymort = 1 - 80000 / ninit[2] # Instead of distributing bycatch mortalities, we concentrate them on one age (1) for lack of length-at-age resolution. 
                              # INAPESCA (2018) - estimated for 2011 - 2017.

```

#####e. Reproduction
* Build reproduction afrom INAPESCA estimates of maturation and egg production by weight.
...
#  Turn lengths into mm.
lmm = l * 10
#  Set function.
fun_fec = function(lmm){1 / (1 + exp(-0.0099 * (lmm - 1333.31)))}
...
# Set up first test.
fun_test = function(nrow, ncol, ninit, v_mort, bymort, v_select_use, v_eggs, k, w){
                                mat_a = matrix(nrow = nrow, ncol = ncol)
                                mat_a[1,] = ninit
                                for(i in 2:nrow){mat_a[i, 3:27] = mat_a[i - 1, 2:26] * v_mort[3:27] * (1 - v_select_use[3:27])
                                                 mat_a[i, 2] = mat_a[i - 1, 1] * v_mort[2] * bymort * (1 - v_select_use[2])
                                                 mat_a[i, 1] = sum(mat_a[i - 1, 1:27] * v_eggs) / (1 + sum(mat_a[i - 1, 1:27] * w) / k)} # Note alpha.
