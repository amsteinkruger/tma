...
# Vector for proportional fecundities at age. From INAPESCA (2018) / Valenzuela-Quiñonez (2014).
#  Set function.
...
fun_eggs = function(l){221.76 * exp(0.0047 * l)} # Valenzuela-Quiñonez (2014); used with Anda-Montañez et al. (2013) for mean by INAPESCA (2018).
...
* hey look it's a model run on vectors instead of functions, fix your shit, me
...
try_sum = 
ggplot(muh, aes(t, s)) +
  geom_segment(aes(x = 2020, xend = Inf, y = 4000, yend = 4000), linetype = "dashed") +
  geom_segment(aes(x = 2020, xend = Inf, y = 2500, yend = 2500), linetype = "dashed") +
  geom_line(size = 3, color = "#1F4E79") +
  scale_x_continuous(expand = c(0, 0), limits = c(2020, 2029), breaks = c(2020, 2030)) +
...
huh = muh %>%
  mutate(b = ifelse(t < 2023, s, s[4]))

try_hum = 
ggplot(huh) +
  geom_segment(aes(x = 2020, xend = Inf, y = 4000, yend = 4000), linetype = "dashed") +
  geom_segment(aes(x = 2020, xend = Inf, y = 2500, yend = 2500), linetype = "dashed") +
  geom_ribbon(aes(t, ymin = s, ymax = b), fill = "#F4B183", alpha = 0.25) +
  geom_line(aes(t, b), color = "#F4B183", size = 3) +
  geom_line(aes(t, s), color = "#1F4E79", size = 3) +
  scale_x_continuous(expand = c(0, 0), limits = c(2020, 2029), breaks = c(2020, 2030)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 10000), labels = scales::comma) +
  ggtitle("Range for Aquaculture Impacts, 2017 - 2027") +
  labs(x = "Year", y = "Tonnes of Biomass") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5, size = 32, family = "Century Gothic", face = "bold"), 
        rect = element_rect(fill = "transparent"),
