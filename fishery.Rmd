...
# Vectors for proportional natural mortalities at age. NB: Survival, not mortality. From INAPESCA (2018) / Cisneros-Mata et al. (1995).
v_mort_first = 0.73
v_mort_rest = rep(0.73, 28)

# Vectors for proportional poaching mortalities at age.

# Vectors for proportional bycatch mortalities at age.

# Vector for proportional fecundities at age. Each fish older than 7 years returns 5 fish. PLACEHOLDER PLACEHOLDER PLACEHOLDER
v_fec_rest = rep(20, 22)

# Plug initial numbers into the matrix.
mat_a[1,] = ninit

# Run one-shot test.
mat_a[2, 2:30] = mat_a[1, 1:29] * v_mort_rest
mat_a[2, 1] = sum(mat_a[1, 8:30] * v_fec_rest)

# Set up first test.
fun_test = function(nrow, ncol, ninit, mort_rate, mort_cols, fec_rate, fec_cols, k){
                                mat_a = matrix(nrow = nrow, ncol = ncol)
                                v_init = ninit
                                v_mort_first = mort_rate
                                v_mort_rest = rep(mort_rate, mort_cols)
                                v_fec_rest = rep(fec_rate, fec_cols)
                                mat_a[1,] = v_init
                                for(i in 2:nrow){mat_a[i, 2:30] = mat_a[i - 1, 1:29] * v_mort_rest
                                                 mat_a[i, 1] = sum(mat_a[i - 1, 8:30] * v_fec_rest)/(1 + mat_a[i - 1, 1] / k)}
                                  
                                return(mat_a)}

# Run first test.
uh = fun_test(25, 30, ninit, 0.73, 28, 5, 22, 5000000)
