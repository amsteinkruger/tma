...
fn_am_marg = function(a, b1, b2){m = b1 * exp(b2 * a * 12)} 

# Set cumulative mortality.
fn_am_cumu = function(a, b1, b2){M = (b1 * exp(b2 * a * 12)) / b2 - (b1 * exp(b2 * 0)) / b2} # Note y-intercept adjustment. Check integration.
...
After mortality, any biomass in excess of optimal stocking density is harvested. Optimal density is 140,000 kilograms per cage at and after first salable size. First salable size is 2 kilograms.
...
fn_ns = function(density, w, a, am_b1, am_b2){ns = density / w * (1 - 0.01 * fn_am_marg(a, am_b1, am_b2))} # Note lag on age to get last year's mortality.
...
m_sale = fn_am_cumu(a_sale, am_b1, am_b2)
...
fn_nm = function(nstart, a, am_b1, am_b2){nm = nstart * (1 - 0.01 * fn_am_marg(a, am_b1, am_b2))}
fn_nt = function(density, w, a, am_b1, am_b2, nstart){nt = ifelse(fn_nm(nstart, a, am_b1, am_b2) - fn_ns(density, w, a, am_b1, am_b2) > 0, 
                                                                  fn_nm(nstart, a, am_b1, am_b2) - fn_ns(density, w, a, am_b1, am_b2), 
                                                                  0)}
...
mp_b1 = -5.28312
mp_b2 = 0.52540
mp_q = 14 # Note that 14 fills in for a reasonable quantity at market from poaching.
...
w = fn_lw(wl_a, l, wl_b)
# This implementation trims stock for optimal density, but does not reflect revenue from trimming.
n = nstart - fn_nt(density, w, a, am_b1, am_b2, nstart)
ymaw = fn_wm(w, n, wm_c, wm_k)
pmaw = fn_mp(w, mp_b0, mp_b1, mp_b2, mp_q)
rmaw = fn_mr(ymaw, pmaw)
rround = fn_fr(w, n, pround)
r = rround + rmaw 
c =  fn_ch0(w, f_b1, f_b2, ch0_cf, n, o)
...
aq = function(par){t = as.numeric(vector(length = par[2]))
                   ...
                   p0[1] = fn_mp(w0[1], par[11], par[12], par[13], par[14])
                   rmaw0[1] = y0[1] * p0[1]
                   ...
                   n1[1] = nstart - fn_nt(par[22], w1[1], a1[1], par[23], par[24], par[25])
                   nt1[1] = fn_nt(par[22], w1[1], a1[1], par[23], par[24], par[25])
                   rt1[1] = fn_fr(w1[1], nt1[1], par[15])
                   y1[1] = fn_wm(w1[1], n1[1], par[9], par[10])
                   p1[1] = fn_mp(w1[1], par[11], par[12], par[13], par[14])
                   rmaw1[1] = y1[1] * p1[1]
                   ...
                   for(i in 2:par[2]){
                     ...
                     n0[i] = nstart - fn_nt(par[22], w0[i], a0[i], par[23], par[24], par[25])
                     nt0[i] = fn_nt(par[22], w0[i], a0[i], par[23], par[24], par[25]) - fn_nt(par[22], w0[i - 1], a0[i - 1], par[23], par[24], par[25])
                     rt0[i] = fn_fr(w0[i], nt0[i], par[15])
                     y0[i] = fn_wm(w0[i], n0[i], par[9], par[10])
                     p0[i] = fn_mp(w0[i], par[11], par[12], par[13], par[14])
                     rmaw0[i] = y0[i] * p0[i]
                     ...
                     n1[i] = nstart - fn_nt(par[22], w1[i], a1[i], par[23], par[24], par[25])
                     nt1[i] = fn_nt(par[22], w1[i], a1[i], par[23], par[24], par[25]) - fn_nt(par[22], w1[i - 1], a1[i - 1], par[23], par[24], par[25])
                     rt1[i] = fn_fr(w1[i], nt1[i], par[15])
                     y1[i] = fn_wm(w1[i], n1[i], par[9], par[10])
                     p1[i] = fn_mp(w1[i], par[11], par[12], par[13], par[14])
                     rmaw1[i] = y1[i] * p1[i]
                     ...
                     h[i] = ifelse(a0[i] > 6, ifelse((r0[i] - c0[i]) > (par[19] * (r1[i] - c1[i] + rt0[i])), 1, 0), 0)
                     hinv[i] = (h[i] - 1) ^ 2
                    ...
                    output = tibble(t, c, r, h, n0, nt0, rt0)
