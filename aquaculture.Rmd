...
fn_am_marg = function(a, b1, b2){m = b1 * exp(b2 * a * 12) + 2.5} 
...
# Note placeholder shift from regression to constant marginal feed requirement.
...
fn_ch0 = function(w, b1, b2, cf, n, o){ch0 = n * cf * fn_f_marg(w, b1, b2) * 365 + o}
...
aq = function(par){t = as.numeric(vector(length = par[2]))
                   ...
                   p0[1] = fn_mp(w0[1], par[11], par[12], par[13], par[14]) * 0.10 # Note price transformation.
                   rmaw0[1] = y0[1] * p0[1]
                   ...
                   p1[1] = fn_mp(w1[1], par[11], par[12], par[13], par[14]) * 0.10 # Note price transformation.
                   rmaw1[1] = y1[1] * p1[1]
                   ...
                   for(i in 2:par[2]){
                     ...
                     p0[i] = fn_mp(w0[i], par[11], par[12], par[13], par[14]) * 0.10 # Note price transformation.
                     rmaw0[i] = y0[i] * p0[i]
                     rround0[i] = fn_fr(w0[i], n0[i], par[15])
                     r0[i] = 0 * rmaw0[i] + rround0[i] # Note the null on maw value.
                     c0[i] = fn_ch0(w0[i], par[16], par[17], par[18], n0[i], par[19]) #+ 10000 * w0[i] ^ 2  | Extra costs for a pretty cost function.
                     ...
                     p1[i] = fn_mp(w1[i], par[11], par[12], par[13], par[14]) * 0.10 # Note price transformation.
                     rmaw1[i] = y1[i] * p1[i]
                     rround1[i] = fn_fr(w1[i], n1[i], par[15])
                     r1[i] = rmaw1[i] + rround1[i]
                     c1[i] = fn_ch0(w1[i], par[16], par[17], par[18], n1[i], par[19]) #+ 10000 * w1[i] ^ 2 | Extra costs for a pretty cost function.
                     
                     # Mind wrapper for hard-coding lower bound age at harvest.
                     h[i] = 0 #ifelse(a0[i] > 2, ifelse((r0[i] - c0[i]) > (par[21] * (r1[i] - c1[i] + rt0[i])), 1, 0), 0)
                     hinv[i] = (h[i] - 1) ^ 2
...
pi_sum = as.numeric(vector(length = par[2] - 1))
pi_sum[1] = 0

for(i in 2:24){pi_sum[i] = tbl_pi$r[i] * tbl_pi$h[i] - tbl_pi$mc[i] + pi_sum[i - 1]} # par[2] for length
