...
nstock = density / w * (1 - 0.01 * fn_am_marg(a, am_b1, am_b2)) # Note lag on age to get last year's mortality.
...
nmort = nstart * (1 - 0.01 * fn_am_cumu(a, am_b1, am_b2))
ntrim = ifelse(nmort - nstock > 0, nmort - nstock, 0)
...
w = fn_lw(wl_a, l, wl_b)
# This implementation trims stock for optimal density, but does not reflect revenue from trimming. Also, there's a mystery NA in n[1].
n = nmort - ntrim
n[1] = nstart
...
aq = function(par){t = as.numeric(vector(length = par[2]))
                   ...
                   n1[1] = par[8] # Fix this to reflect change from constant n to n with mortality and trimming.
